...
// Data structures for events and news with unique links
let events = [
    { title: "Sosialisasi", link:"sosialisasi.html", date: "Jumat, 15 Nov 2024 <br> 01:47 WIB", img: "politik.png", link: "sosialisasi.html" },
    { title: "Sosialisasi KPU",link:"sosialisasiKPU.html", date: "Jumat, 15 Nov 2024 <br> 01:47 WIB", img: "politik.png", link: "sosialisasi-kpu.html" },
    { title: "Pencalonan Kandidat",link:"sosialisasi.html", date: "Senin, 18 Nov 2024 <br> 10:30 WIB", img: "politik2.png", link: "pencalonan-kandidat.html" }
];

let news = [
    { title: "Sosialisasi KPU", date: "Jumat, 15 Nov 2024 <br> 01:47 WIB", img: "politik.png", link: "sosialisasi-kpu-news.html" },
    { title: "Hasil Pencalonan", date: "Senin, 18 Nov 2024 <br> 11:00 WIB", img: "politik2.png", link: "hasil-pencalonan-news.html" }
];
...
function renderAcara() {
    ...
    `).join('') + `<div class="more"><a href="acara.html">SELENGKAPNYA</a></div>`;
}

// Render function for BERITA section
function renderBerita() {
    const beritaContainer = document.getElementById("beritaAber");
    beritaContainer.innerHTML = news.map(newsItem => `
        <div class="news">
            <img src="${newsItem.img}" alt="News Image">
            <div>
                <p><a href="${newsItem.link}">${newsItem.title}</a></p>
                <span>${newsItem.date}</span>
            </div>
        </div>
    `).join('') + `<div class="more"><a href="berita.html">SELENGKAPNYA</a></div>`;
}
...
function addEvent(title, date, img, link) {
    events.push({ title, date, img, link });
    renderAcara();
    renderBerita();
}
...
function updateEvent(index, title, date, img, link) {
    if (events[index]) {
        events[index] = { title, date, img, link };
        renderAcara();
        renderBerita();
    }
}
